...
def calculate_moving_average_crossover(df, ma_short, ma_long):
    # Calcula o RSI
...
def process_csv_file(file_path, ma_short, ma_long):
    df = pd.read_csv(file_path)
    efficiency_gain = calculate_moving_average_crossover(df, ma_short, ma_long)
    return efficiency_gain


if __name__ == '__main__':
    #Definir o periodo que vocÊ quer para o rsi
    calculate_rsi(15)
    ...
    with Pool(num_processes) as p:
        results = [p.apply_async(process_csv_file, args=(f, ma_short, ma_long)) for f in csv_files]
    ...
    # Define outros períodos das médias móveis
    print(f'Análise feita com outras configurações de médias móveis')
    ma_short_2 = 60
    ma_long_2 = 80

    # Processa os arquivos CSV usando multiprocessing com outras médias móveis
    with Pool(num_processes) as p:
        results = [p.apply_async(process_csv_file, args=(f, ma_short_2, ma_long_2)) for f in csv_files]
        efficiency_gains_2 = [(file_path, result.get()) for file_path, result in zip(csv_files, results)]

    # Exibe os resultados
    print("\nGanho de eficiência para outras médias móveis:")
